...
Два ответа показывают экран-реакцию — он не задаёт вопросов и не попадает
в счётчик прогресса:

| Когда | Что говорим |
|---|---|
| выбран 5 или 6 класс | у младших классов программа устроена иначе: без экзамена, по школьной программе |
| выбрано «Уже в Умскул» | не будем предлагать оплаченный курс — уточним текущие предметы |

Формулировки лежат в объекте `NOTICE_TEXT` в начале `assets/app.js` — тексты
черновые, правятся без изменения логики.
...
- Наборы вопросов по веткам — объект `TRACK_STEPS` в `assets/app.js`: чтобы добавить
  ...
  (отбивки и вопрос действующим ученикам) подставляет функция `steps()`.
- Тексты отбивок — объект `NOTICE_TEXT`.
