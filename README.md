...
| 3 | Какие предметы будет сдавать (в 7–8 классе — по каким интересует подготовка) | мультивыбор + «Пока не знаю» | обе |
...
10–11 → ЕГЭ (ползунок 40–100 баллов), 7–8 → школьная программа (оценка 3–5).
В 7–8 классе сдавать ещё нечего, поэтому и вопрос другой — «По каким предметам
интересует подготовка?», — и список короче:

| Класс | Предметы |
|---|---|
| 7 | математика, русский, физика, история, биология |
| 8 | то же + информатика, химия, английский |

Список задаётся объектом `SCHOOL_SUBJECTS` в `assets/app.js`, порядок в нём —
порядок карточек на экране. При смене класса выбор, которого нет в новом наборе,
...
- Палитра и радиусы — переменные в `:root` в `assets/styles.css`
  (`--brand`, `--accent`, `--radius`).
- Списки предметов, вариантов подготовки и классов — константы `SUBJECTS`,
  `SCHOOL_SUBJECTS`, `PREP`, `GRADES` в начале `assets/app.js`.
